...
function main() {
  ...
  cd "$ROOT/data/pb" &> /dev/null

  generate "hivemapper/v1/hivemapper.proto"
...
function generate() {
    ...
    for file in "$@"; do
      protoc -I$PROTO_HIVEMAPPER \
        --go_opt=Mhivemapper/v1/hivemapper.proto=github.com/streamingfast/honey-tracker/data/pb \
